...
pub fn print_cli_auth_help() {
    ...
    println!("==========================");
    println!();
    println!("Persistent login (interactive web flow):");
    println!("  pcloud-cli auth login");
    println!();
    println!("Non-interactive token login (token persisted):");
    println!("  pcloud-cli auth login --token YOUR_AUTH_TOKEN");
    println!();
    println!("Ephemeral env-var auth (never saved):");
    println!("  PCLOUD_AUTH_TOKEN=<token> pcloud-cli mount");
    println!("  PCLOUD_AUTH_TOKEN_FILE=/run/secrets/token pcloud-cli start");
    println!();
    println!("Mount with a one-shot token (foreground / background):");
    println!("  pcloud-cli mount /mnt/pcloud --token YOUR_AUTH_TOKEN");
    println!("  pcloud-cli start /mnt/pcloud --token YOUR_AUTH_TOKEN");
    println!();
    println!("Crypto unlock (after a daemon is running):");
    println!("  PCLOUD_CRYPTO_PASS=<password> pcloud-cli crypto start");
    println!("  pcloud-cli crypto start --password-file /run/secrets/crypto");
    println!();
    println!("Clear saved credentials:");
    println!("  pcloud-cli auth logout");
    println!();
    println!("Clear credentials AND all local sync data (destructive):");
    println!("  pcloud-cli auth unlink");
    println!();
    println!("Default mount path: ~/pCloud (override with $PCLOUD_MOUNTPOINT or PATH arg)");
    println!();
}
